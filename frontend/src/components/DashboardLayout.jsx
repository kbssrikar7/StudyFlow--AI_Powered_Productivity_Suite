...
const DashboardLayout = () => {
    ...
    return (
        <div className="flex h-screen overflow-hidden bg-[#050505] text-slate-100 font-sans selection:bg-red-900/30 selection:text-red-200">
            ...
            {/* Sidebar */}
            <aside className="w-72 flex-shrink-0 border-r border-white/5 bg-[#0a0a0a] p-6 flex flex-col relative overflow-hidden z-10">
                ...
                {/* Brand */}
                <div className="flex items-center justify-center mb-10 relative z-10 px-2 pt-4">
                    <div className="relative group">
                        <div className="absolute inset-0 bg-red-600/10 blur-xl rounded-full opacity-0 group-hover:opacity-100" />
                        <img
                            src="/bat-logo-circular.png"
                            alt="Batman Logo"
                            className="w-24 h-24 object-contain relative z-10 opacity-100 drop-shadow-[0_0_10px_rgba(0,0,0,0.8)]"
                        />
                    </div>
                </div>

                {/* Navigation */}
                <nav className="flex-1 flex flex-col gap-2 overflow-y-auto relative z-10 -mx-2 px-2">
                    {tabs.map((tab) => {
                        const Icon = tab.icon;
                        ...
                        return (
                            <button
                                ...
                                className={`group flex items-center gap-4 rounded-xl px-4 py-3.5 text-left border ${isActive
                            ...
                            >
                                <Icon
                                    size={20}
                                    className={`${isActive ? 'scale-110 drop-shadow-[0_0_8px_rgba(220,38,38,0.5)]' : 'group-hover:scale-110'}`}
                                    strokeWidth={isActive ? 2.5 : 2}
                                />
                                <div>
                                    <p className={`text-sm font-semibold tracking-wide ${isActive ? 'text-red-100' : ''}`}>{tab.label}</p>
                                    <p className={`text-[10px] ${isActive ? 'text-red-500/70' : 'text-slate-600'}`}>{tab.description}</p>
                                </div>
                                {isActive && (
                ...
                <div className="mt-8 pt-6 border-t border-white/5 relative z-10">
                    <div className="flex items-center gap-3 mb-4 px-2">
                        <div className="w-10 h-10 rounded-full bg-gradient-to-br from-slate-800 to-black border border-white/10 flex items-center justify-center text-slate-400 shadow-inner">
                            <Shield size={18} />
                        </div>
                        <div className="overflow-hidden">
                            <p className="text-sm font-bold text-slate-200 truncate">{user?.full_name || 'Bruce Wayne'}</p>
                            <p className="text-[10px] text-slate-500 truncate font-mono">ID: {user?.email?.split('@')[0].toUpperCase() || 'BW-001'}</p>
                        </div>
                    </div>

                    <button
                        onClick={logout}
                        className="w-full flex items-center justify-center gap-2 rounded-xl border border-white/5 bg-white/5 px-4 py-3 text-xs font-semibold uppercase tracking-wider text-slate-400 transition-all hover:bg-red-500/10 hover:text-red-400 hover:border-red-500/20 group"
                    >
            ...
            <main className="flex-1 overflow-y-auto bg-transparent relative scroll-smooth z-10">
                {/* Top Bar / Header */}
                <header className="sticky top-0 z-20 bg-[#050505]/95 border-b border-white/5 px-8 py-4 flex items-center justify-between">
                    <div>
                        <h2 className="text-2xl font-bold text-white tracking-tight">{currentTab?.label}</h2>
                    </div>
                </header>
